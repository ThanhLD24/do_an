...
<body>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			...
			<li class="active"><a href="<%=request.getContextPath()%>/admin/account"><i class="uk-icon-users"
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<div class="row">
						<div class="col-md-3">
					<button type="button" class="btn btn-primary" onclick="location.href = '<%=request.getContextPath()%>/admin/add-account';">Thêm tài khoản</button>
							</div>
					...
					<div class="panel-body">
						<table data-toggle="table" data-show-refresh="false"
							...
							<tbody>
								<%
									List<Account> listAccount = ObjectManager.listObject(Account.class);
									Employee emp = null;
									Permit permit = null;
									for (Account acc : listAccount) {
										emp = (Employee) ObjectManager.getObjectById(acc.getEmployeeId(), Employee.class);
										permit = (Permit) ObjectManager.getObjectById(acc.getAccountPermit(), Permit.class);
								%>
								<tr>
									<td><%=acc.getEmployeeId()%></td>
									...
									<td align="center"><p data-placement="top"
											...
											<button class="btn btn-primary btn-xs" data-title="Edit"
												data-toggle="modal"
												data-target="#edit<%=acc.getAccountId()%>"
												style="height: 22px">
										...
										<p data-placement="top" data-toggle="tooltip" title="Xóa"
											...
											<button class="btn btn-danger btn-xs" data-title="Xóa"
												data-toggle="modal"
												data-target="#delete<%=acc.getAccountId()%>"
												style="height: 22px">
								...
								<!-- popup edit -->
								<div class="modal fade" id="edit<%=acc.getAccountId()%>"
												...
												<div class="modal-body">
													<div class="form-group">
														<input class="form-control " type="hidden"
															value="<%=acc.getAccountId()%>" name="txtAccountId"
															id="txtAccountId"> <input class="form-control "
															type="text" placeholder="Tên tài khoản"
															name="txtAccountName" id="txtAccountName"
															value="<%=acc.getAccountName()%>">
													</div>
													<div class="form-group">

														<input class="form-control " type="text"
															placeholder="Mật khẩu" name="txtAccountPassword"
															id="txtAccountPassword"
															value="<%=acc.getAccountPassword()%>">
													</div>
													<div class="form-group">

														<select class="form-control" name="txtAccountPermit"
															id="txtAccountPermit">
															<option disabled="disabled" >=== Chọn
																quyền ===</option>
															<%
																for (Permit per : (List<Permit>) ObjectManager.listObject(Permit.class)) {
															%>
															<option value="<%=per.getPermitId()%>" <%if(per.getPermitId()==permit.getPermitId()) {%>selected="selected"<%} %>><%=per.getPermitName()%></option>
															<%
																}
															%>
														</select>
								...
								<div class="modal fade" id="delete<%=acc.getAccountId()%>"
									tabindex="-1" role="dialog" aria-labelledby="edit"
									aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-body">
												...
												<div class="alert alert-info">
													...
													nhận xóa tài khoản ID:<b><%=acc.getAccountId()%></b>?
												</div>

											</div>
											<form id="formDeleteBook"
												action="<%=request.getContextPath()%>/AdminBusController?type=<%=Variables.DELETE_ACCOUNT%>"
												method="post">
												<input class="form-control " type="hidden"
													value="<%=acc.getAccountId()%>" name="txtAccountId"
													id="txtAccountId">
