...
<div class="panel-heading" style="line-height: 0px;">
					...
					<div class="row">
						<div class="col-md-3">
					<button type="button" class="btn btn-primary" onclick="location.href = '<%=request.getContextPath()%>/admin/add-bus';">Thêm xe</button>
							</div>
						...
						<table data-toggle="table" data-show-refresh="false"
							...
							<thead>
								<tr>
									...
									<th data-sortable="true">Mã xe</th>
									<th data-sortable="true">Biển kiểm soát</th>
									<th data-sortable="true">Loại ghế</th>
									<th data-sortable="true">Số chỗ</th>
									<th data-sortable="true">Tiện nghi</th>
									<th data-sortable="true">Tên xe</th>
									<th data-sortable="true">Nhà xe</th>
									<th data-sortable="true">Hãng xe</th>
							...
							<tbody>
								<%
									...
									Carrier carr = null;
									for (Bus bus : listBus) {
										carr = (Carrier) ObjectManager.getObjectById(bus.getCarrierId(), Carrier.class);
								%>
								<tr>
									<td><%=bus.getBusId()%></td>
									<td><%=bus.getBusLicensePlate()%></td>
									<td><%=bus.getBusType()%></td>
									<td><%=bus.getBusCapacity()%></td>
									<td><%=bus.getBusFeature()%></td>
									<td><%=bus.getBusName()%></td>
									<td><%=carr.getCarrierName()%></td>
									<td><%=bus.getBusManufacturer()%></td>
									<td>
										<%
											if (bus.getBusState() == Variables.INVALID) {
										%><h4>
											<span class="label label-warning">Không sử dụng</span>
										</h4> <%
 	}
 		if (bus.getBusState() == Variables.VALID) {
 %> <span class="label label-success">Đang lưu hành</span>
										<%
											}
										%>
									</td>
									<td align="center"><p data-placement="top"
											data-toggle="tooltip" title="Xem bản đồ xe"
											style="display: inline">
											<button class="btn btn-primary btn-xs" data-title="Edit"
												data-toggle="modal" data-target="#edit<%=bus.getBusId()%>"
												style="height: 22px">
										...
										<p data-placement="top" data-toggle="tooltip" title="Xóa"
											...
											<button class="btn btn-danger btn-xs" data-title="Xóa"
												data-toggle="modal" data-target="#delete<%=bus.getBusId()%>"
												style="height: 22px">
								...
								<!-- popup edit -->
								<div class="modal fade" id="edit<%=bus.getBusId()%>"
									...
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												...
												<h4 class="modal-title custom_align" id="Heading">Bản
													đồ xe biển kiểm soát <b><%=bus.getBusLicensePlate() %></b></h4>
											</div>
												...
												<div class="form-group">
													<div class="row">
														<div class="col-md-12">
															<div class="wrapper">
																<div class="container">
																	<div id="seat-map-<%=bus.getBusId()%>">
																		<div class="front-indicator">Tang 1</div>

																	</div>
																	<div id="seat-map1-<%=bus.getBusId()%>">
																		<div class="front-indicator">Tang 2</div>
			...
			$(document).ready(function() {
				var $cart = $('#selected-seats'),
					...
					firstSeatLabel = 1,
					$total = $('#total'),
					sc = $('#seat-map-<%=bus.getBusId()%>').seatCharts({
				...
				sb = $('#seat-map1-<%=bus.getBusId()%>').seatCharts({
					map: [<%=Utility.replaceString(bus.getBusMap())%>
							...
							return firstSeatLabel++;
											...
											<div class="modal-footer ">
												<button type="button" class="btn btn-success btn-lg"
													style="width: 100%;" id="btEdit" data-dismiss="modal">
								...
								<div class="modal fade" id="delete<%=bus.getBusId()%>"
									tabindex="-1" role="dialog" aria-labelledby="edit"
									aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											...
											<div class="modal-header">
												...
												<h4 class="modal-title custom_align" id="Heading">Chuyển trạng thái</h4>
											</div>
											<div class="modal-body">

												<div class="alert alert-info">
													<span class="glyphicon glyphicon-info-sign"></span> Chuyển trạng thái xe BKS <b><%=bus.getBusLicensePlate() %></b> sang <b>Không sử dụng</b>
												</div>

											</div>
											<form id="formChange"
												action="<%=request.getContextPath()%>/AdminBusController?type=<%=Variables.CHANGE_BUS_STATUS%>"
												method="post">
												<input class="form-control " type="hidden"
													value="<%=bus.getBusId()%>" name="txtBusId"
													id="txtBusId">
												<div class="modal-footer ">
													<button type="submit" class="btn btn-success">
														<span class="glyphicon glyphicon-ok-sign"></span> Chuyển
													</button>
